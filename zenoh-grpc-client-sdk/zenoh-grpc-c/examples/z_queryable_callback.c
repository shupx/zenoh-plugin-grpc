...
static void on_query(zgrpc_queryable_t* queryable, const zgrpc_query_ex_t* query, void* context) {
    (void)context;
    ...
    const char* reply1 = "this is a reply1 from c queryable callback";
    ...
    const char* reply2 = "this is a reply2 from c queryable callback";
    zgrpc_queryable_reply_with_options(queryable, query->query_id, query->key_expr,
...
int main(int argc, char** argv) {
    ...
    zgrpc_queryable_t* queryable = zgrpc_declare_queryable_with_callback(
        session, "demo/query/**", on_query, NULL, NULL, &opts);
    if (queryable == NULL) {
        fprintf(stderr, "failed to declare callback queryable\n");
        zgrpc_close(session);
        return 1;
    }

    printf("callback queryable ready on %s\n", endpoint);
    zgrpc_example_sleep_ms(10000);
